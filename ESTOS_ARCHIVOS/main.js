...
class IQOptionAIScanner {
  constructor() {
    ...
    this.tradingConfig = {
      ...
      minConfidence: 50, // Confianza mínima para ejecutar (50%)
  ...
  async coordinateTrading(prediction) {
    try {
      ...
      // EJECUTAR cuando hay 1+ señales únicas, no hay operaciones activas Y estamos en el timing correcto
      if (finalSignals.length >= 1 && this.activeOperations.size === 0 && shouldExecute) {
        ...
        // CALCULAR TIMING EXACTO PARA 6 SEGUNDOS ANTES DEL :00 (compensar delay)
        const now = new Date();
        const currentSeconds = now.getSeconds();
        const currentMilliseconds = now.getMilliseconds();
        
        // EJECUTAR 6 segundos ANTES del próximo minuto (54 segundos del minuto actual)
        const nextMinute = new Date(now);
        nextMinute.setMinutes(nextMinute.getMinutes() + 1);
        nextMinute.setSeconds(0);
        nextMinute.setMilliseconds(0);
        
        // RESTAR 6 segundos para compensar el delay
        const targetTime = new Date(nextMinute.getTime() - 6000); // 6 segundos antes
        
        const millisecondsToTarget = targetTime.getTime() - now.getTime();
        
        this.logger.info(`⏰ COORDINADOR: Ejecutar EXACTAMENTE a los :00 segundos en ${Math.round(millisecondsToTarget / 1000)}s (${targetTime.toLocaleTimeString()})`);
        
        // Programar ejecución EXACTA a los :00 segundos
        setTimeout(async () => {
          try {
            ...
            // Verificar que estamos ejecutando cerca de los :00 segundos
            if (actualSeconds > 3) {
              this.logger.warn(`⚠️ COORDINADOR: Ejecución tardía - Segundos: ${actualSeconds} (debería ser 0-2)`);
            }
