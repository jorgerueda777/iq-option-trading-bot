...
def execute_dynamic_trades(signals_data):
    ...
    try:
        ...
        def execute_single_trade(signal):
            ...
            if is_even_minute:
                # MINUTO PAR: EJECUTAR IGUAL
                direction = original_direction
                if original_direction == "call":
                    direction_text = "COMPRA (IGUAL)"
                else:
                    direction_text = "VENTA (IGUAL)"
                strategy_mode = "IGUAL"
            else:
                # MINUTO IMPAR: EJECUTAR INVERSO
                if original_direction == "call":
                    direction = "put"
                    direction_text = "VENTA (INVERSO)"
                    strategy_mode = "INVERSO"
                else:
                    direction = "call"
                    direction_text = "COMPRA (INVERSO)"
                    strategy_mode = "INVERSO"
